        ...
        public static GMapMarker getMAVMarker(MAVState MAV)
        {
            ...
            if (MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)
            {
                ...
                { 
                    ...
                    // CurrentState.SpeedUnit + " | 自由者0580" ,//+ (int) MAV.sysid,
                    ToolTipText = /*"自由者0580"+"\n"+*/
                                  "目标航点："+(int )MAV.cs.wpno + "\n"+
                                  "高度：" + (int)MAV.cs.alt + CurrentState.AltUnit +"\n"+
                                  "地速：" + (int)MAV.cs.groundspeed+ CurrentState.SpeedUnit+"\n"+
                                  "空速：" + (int)MAV.cs.airspeed + CurrentState.SpeedUnit + "\n"+
                                  "离家距离："+ (int )MAV.cs.DistToHome + "\n" +
                                  "飞行里程："+ (int)MAV.cs.distTraveled,
