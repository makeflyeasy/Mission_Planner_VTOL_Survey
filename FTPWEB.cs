...
namespace FtpTest1
{
    public class FtpWeb
    {
        ...
        public string Download1(string filePath, string fileName)
        {
            FtpWebRequest reqFTP;
            ...
            try
            {
                ...
                Progress.progressBar1.Value = Progress.progressBar1.Minimum = 0;
                Progress.Text = "正在下载更新包";
                Progress.Show();
                while (readCount > 0)
                {
                    outputStream.Write(buffer, 0, readCount);
                    readCount = ftpStream.Read(buffer, 0, bufferSize);
                    size = new FileInfo(filePath + fileName).Length;
                    Progress.progressBar1.Value = (int )(size / (float)FileSize*100);
                }
                ftpStream.Close();
                outputStream.Close();
                response.Close();
