...
namespace MissionPlanner.Controls
{
    public partial class SITL : MyUserControl, IActivate
    {
        ...
        private Regex default_params_regex = new Regex(@"""([^""]+)""\s*:\s*\{\s*[^\{}]+""default_params_filename""\s*:\s*\[*""([^""]+)""\s*[^\}]*\}");

        private Uri sitlurl = new Uri("http://firmware.ardupilot.org/Tools/MissionPlanner/sitl/");

        private string sitldirectory = Settings.GetUserDataDirectory() + "sitl" +
                               Path.DirectorySeparatorChar;

        private GMapOverlay markeroverlay;

        private GMapMarkerWP homemarker = new GMapMarkerWP(new PointLatLng(-34.98106, 117.85201), "H");
    ...
    { "calibration",        Calibration::create },
        ...
        private string BuildHomeLocation(PointLatLng homelocation, int heading = 0)
        {
        ...
        private void cleanupJson(string filename)
        {
        ...
        private static string BraceMatch(string text, char braces, char bracee)
        {
            int level = 0;
            int start = 0;
            int end = 0;
