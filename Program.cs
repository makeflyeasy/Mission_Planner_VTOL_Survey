...
namespace MissionPlanner
{
 ...
 //               Splash.pictureBox1.Visible = false;
           ...
           /* string strVersion = File.Exists("version.txt")
            ...
            Splash.Text = "Mission Planer Vtol Survey V1.6（测试版）"; //name/*+ Application.ProductVersion*/+" V1";// + " build " + strVersion;
            Splash.Show();
        ...
        static void handleException(Exception ex)
        {
            ...
            if (ex.GetType() == typeof (ObjectDisposedException) || ex.GetType() == typeof (InvalidOperationException))
                // something is trying to update while the form, is closing.31-
            {
                log.Error(ex);
                return; // ignore
            }
