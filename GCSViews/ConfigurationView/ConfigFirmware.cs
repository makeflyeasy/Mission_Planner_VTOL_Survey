...
namespace MissionPlanner.GCSViews.ConfigurationView
{
    partial class ConfigFirmware : MyUserControl, IActivate, IDeactivate
    {
        ...
        private void picturebox_Click(object sender, EventArgs e)
        {
            ...
            firmware.Progress -= fw_Progress;
            firmware.Progress += fw_Progress1;
            var boardtype = BoardDetect.boards.none;

            try
            {
                /*if (filepath.ToLower().EndsWith(".apj"))
                {
                    if (solo.Solo.is_solo_alive &&
                        CustomMessageBox.Show("Solo", "Is this a Solo?", CustomMessageBox.MessageBoxButtons.YesNo) == CustomMessageBox.DialogResult.Yes)
                    {
            ...
            var filepath="";
            FtpWeb fw = new FtpWeb("47.108.137.134", "firmware", "doc", "bfxKe68Erv4qB");
            string[] fileList = fw.GetFileList("*.*");
            if (boardtype.ToString() == "px4v2" || boardtype.ToString() == "fmuv2")
            {
                filepath = Settings.GetRunningDirectory() + "ArduPlane_FMU_V2.apj";
                for (int a = 0; a < fileList.Length; a++)
                    if (fileList[a] == "ArduPlane_FMU_V2.apj")
                        fw.Download(Settings.GetRunningDirectory(), fileList[a]); 
            }
            else if (boardtype.ToString() == "px4v3" || boardtype.ToString() == "fmuv3")
            {
                filepath = Settings.GetRunningDirectory() + "ArduPlane_FMU_V3.apj";
                for (int a = 0; a < fileList.Length; a++)
                    if (fileList[a]=="ArduPlane_FMU_V3.apj")
                        fw.Download(Settings.GetRunningDirectory(), fileList[a]);
            }
            else if (boardtype.ToString() == "px4v4" || boardtype.ToString() == "fmuv4")
            {
                filepath = Settings.GetRunningDirectory() + "ArduPlane_FMU_V4.apj";
                for (int a = 0; a < fileList.Length; a++)
                    if (fileList[a] == "ArduPlane_FMU_V4.apj")
                        fw.Download(Settings.GetRunningDirectory(), fileList[a]);
            }
            else if (boardtype.ToString() == "px4v5" || boardtype.ToString() == "fmuv5" || boardtype.ToString() == "chbootloader")
            {
                filepath = Settings.GetRunningDirectory() + "ArduPlane_FMU_V5.apj";
                for (int a = 0; a < fileList.Length; a++)
                    if (fileList[a] == "ArduPlane_FMU_V5.apj")
                        fw.Download(Settings.GetRunningDirectory(), fileList[a]);
            }
            firmware.UploadFlash(MainV2.comPortName, filepath, boardtype);
        ...
        private void picturebox1_Click(object sender, EventArgs e)
        {
            ...
            FtpWeb fw = new FtpWeb("47.108.137.134", "firmware", "doc", "bfxKe68Erv4qB");
            string[] fileList = fw.GetFileList("*.*");
             if (boardtype.ToString() == "px4v3"|| boardtype.ToString() == "px4v2" || boardtype.ToString() == "fmuv2" || boardtype.ToString() == "fmuv3")
            {
                filepath = Settings.GetRunningDirectory() + "arduplane_fmu_v3.apj";
                for (int a = 0; a < fileList.Length; a++)
                    if (fileList[a] == "arduplane_fmu_v3.apj")
