    ...
    public partial class ConfigRawParams : MyUserControl, IActivate, IDeactivate
    {
        ...
        public ConfigRawParams()
        {
            ...
            InitializeComponent();
             ...
             Type dgvType = Params.GetType();
            PropertyInfo pi = dgvType.GetProperty("DoubleBuffered",
                BindingFlags.Instance | BindingFlags.NonPublic);
            pi.SetValue(Params, true, null);
            ...
            try
            {
                if (paramfiles == null)
                {
                   // paramfiles = GitHubContent.GetDirContent("ardupilot", "ardupilot", "/Tools/Frame_params/", ".param");
                    FtpWeb fw = new FtpWeb("47.108.137.134", "param", "doc", "bfxKe68Erv4qB");

                     fileList = fw.GetFileList("*.*");
                }

                BeginInvoke((Action)delegate
               {
                   CMB_paramfiles.DataSource = fileList;
        ...
        private void BUT_paramfileload_Click(object sender, EventArgs e)
        {
            ...
            var filepath = Settings.GetRunningDirectory() + CMB_paramfiles.Text;
            try
            {
                FtpWeb fw = new FtpWeb("47.108.137.134", "param", "doc", "bfxKe68Erv4qB");
                /*var data = 
                    GitHubContent.GetFileContent("ardupilot", "ardupilot",
                    ((GitHubContent.FileInfo)CMB_paramfiles.SelectedValue).path);*/
                
            string status=fw.Download(Settings.GetRunningDirectory(), CMB_paramfiles.Text);
        ...
        private void Params_Paint(object sender, PaintEventArgs e)
        {
            Invalidate();
        }
    }
