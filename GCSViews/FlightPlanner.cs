...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        //public double A { get => b; set => b  = value; }
        ...
        public void SetMouseDisplay(double lat, double lng, int alt)
        {
            ...
            label32.Parent = MainMap;
            label32.Text = "纬度：" + coords1.Lat.ToString("0.#######");
            coords1.Lng = mouseposdisplay.Lng;
            label33.BackColor = Color.Transparent;
            label33.Parent = MainMap;
            label33.Text = "经度：" + coords1.Lng.ToString("0.#######");
            ...
            label35.Parent = MainMap;
            label35.Text = "高度：" + coords1.Alt.ToString("0.##") + "m";
        ...
        public void AddWPToMap(double lat, double lng, int alt)
        {
            ...
            //Commands.FirstDisplayedScrollingRowIndex = Commands.Rows[selectedrow].Index-1;
        ...
        private void FlightPlanner_Load(object sender, EventArgs e)
        {
            ...
            if (Settings.Instance["FP_docking"] == "Right")
            {
        ...
        private void TXT_homealt_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                // B= double.Parse(TXT_homealt.Text);
                MainV2.comPort.MAV.cs.HomeLocation.Alt = double.Parse(TXT_homealt.Text);
                B = MainV2.comPort.MAV.cs.HomeLocation.Alt;
            }
            catch (Exception ex)
            {
                log.Error(ex);
        ...
        // polygons
        private GMapPolygon wppolygon;

        internal GMapPolygon drawnpolygon;
        private GMapPolygon geofencepolygon;

        // layers
        private GMapOverlay top; // not currently used
        ...
        public static GMapOverlay polygonsoverlay1;
                ...
                if (item is GMapMarkerRect)
                {
                        ...
                        int.TryParse(rc.InnerMarker.Tag.ToString(), out answer))
                    {
                        try
                        {
                            ...
                            abs = Math.Abs(Convert.ToDouble(Commands[Grad.Index, answer - 1].Value)); //((double)Commands[Grad.Index, answer - 1].Value);
                            if (wpinformantion)
                            {
                                //if (answer != 1)
                                {
                                    if ((Commands[Command.Index, answer - 1].Value.ToString()) != MAVLink.MAV_CMD.LOITER_TO_ALT.ToString())
                                    //double abs =Math .Abs  ((double)Commands[Grad.Index, answer - 1].Value);
                                    {
                                        if (answer - 1 != 1)
                                            if (abs <= 30)
                                            {
                                                ...
                                                item.ToolTipText = "高度:" + Commands[Alt.Index, answer - 1].Value +
                                                        "\n" + "坡度:" + Commands[Grad.Index, answer - 1].Value + "%" +
                                                        "\n" + "距离:" + Commands[Dist.Index, answer - 1].Value +
                                                        "\n" + "命令:" + Commands[Command.Index, answer - 1].Value;
                                                if (wpinformantiontime)
                                                    item.ToolTipMode = MarkerTooltipMode.Always;
                                                else item.ToolTipMode = MarkerTooltipMode.OnMouseOver;
                                            }
                                            else
                                            {
                                                //polygonsoverlay.Markers.Remove ();
                                                //polygonsoverlay.Markers.Add(new GMarkerGoogle(new PointLatLng(Convert.ToDouble (Commands[Lat .Index, answer - 1].Value), Convert.ToDouble(Commands[Lon .Index, answer - 1].Value)), GMarkerGoogleType.red));
                                                item.ToolTipText = "高度:" + Commands[Alt.Index, answer - 1].Value +
                                                      "\n" + "请降低坡度" +
                                                      "\n" + "距离:" + Commands[Dist.Index, answer - 1].Value +
                                                      "\n" + "命令:" + Commands[Command.Index, answer - 1].Value;
        ...
        */
        ...
        private void MainMap_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (polyicon.Rectangle.Contains(e.Location))
            {
                ...
                if (e.Button == MouseButtons.Right)
                {
                    ...
                    but_Delsurveyarea_Click(this, null);
        ...
        {
            MethodInvoker m = delegate { lbl_status.Text = "状态: 加载地图..."; };
            try
            {
                if (IsHandleCreated)
                    BeginInvoke(m);
        ...
        private void myButton1_Click(object sender, EventArgs e)
        {
            try
            {
                 if (!Fristpoint.IsEmpty && !Secondpoint.IsEmpty)
                     CustomMessageBox.Show("距离: " +
                                           FormatDistance(
                                               MainMap.MapProvider.Projection.GetDistance(Fristpoint, Secondpoint), true) +
                                           " 方位角: " +
                                           ((MainMap.MapProvider.Projection.GetBearing(Fristpoint, Secondpoint) - 360)
                                               .ToString("0")));

                 //int a = Commands.Rows.GetLastRow(0);
                 // Commands.Rows.RemoveAt(a);
                 //MouseDownStart.ToString();*/
                if (startmeasure.IsEmpty)
                {
                    startmeasure = MouseDownStart;
                    //MouseDownStart = ;
                    polygonsoverlay.Markers.Add(new GMarkerGoogle(MouseDownStart, GMarkerGoogleType.blue));
                    ...
                    //endmeasure = MouseDownStart;
                    polygonPoints1.Add(startmeasure);
                    polygonPoints1.Add(MouseDownStart);

                    GMapPolygon line = new GMapPolygon(polygonPoints1, "measure dist");
        ...
        private void myButton6_Click_1(object sender, EventArgs e)
        {
            string heading = "0";
        ...
        private void but_Delsurveyarea_Click(object sender, EventArgs e)
        {
            polygongridmode = false;
        ...
        private void clearmission_Click(object sender, EventArgs e)
        {
            //drawnpolygonsoverlay.Markers.Clear();
            quickadd = true;
            takeoff_land = false;
            Grid.GridUI.airline = false;
            // mono fix
            try
            {
                Commands.CurrentCell = null;
        ...
        private void MainMap_Paint(object sender, PaintEventArgs e)
        {
            ...
            //if (SH > 800)
            tabControl1.Height = SH - 200;
            tabControl2.Height = SH - 300;
            Commands.Height = tabControl2.Height - 40;
            // draw utm grid
            if (grid)
            {
                if (MainMap.Zoom < 10)
                ...
                if (Math.Abs(deltax) / 1 < 40)
                    gridsize = 1;
                label36.Parent = MainMap;
                label36.Visible = true;
                label36.Text = "比例尺： 1:"+gridsize ;
                // round it - x
        ...
        private void but_3_dcheck(object sender, EventArgs e)
        {

            try
            {

                SaveFileFromUrl("network.kml", "http://127.0.0.1:56781/network.kml");
                // SaveBinaryFile();

                Process.Start(Settings.GetUserDataDirectory ()+ "network.kml");
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        {
            if (panelAction.Dock == DockStyle.Right)
            {
        ...
        private void CMB_altmode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                currentaltmode = (altmode)CMB_altmode.SelectedValue;
                 ...
                 else if(CMB_altmode.Text =="Absolute")
                         radioButton2.Checked = true;
                 else if (CMB_altmode.Text == "Terrain")
        ...
        private void but_writewpfast_Click(object sender, EventArgs e)
        {
            ...
            IProgressReporterDialogue frmProgressReporter1 = new ProgressReporterDialogue
            {
                StartPosition = FormStartPosition.CenterScreen,
                Text = "Sending WP's"
            };

            frmProgressReporter1.DoWork += saveWPs;
            frmProgressReporter1.UpdateProgressAndStatus(-1, "Sending WP's");

            ThemeManager.ApplyThemeTo(frmProgressReporter1);

            frmProgressReporter1.RunBackgroundOperationAsync();

            frmProgressReporter1.Dispose();
        ...
        private void panel8_CloseClick(object sender, EventArgs e)
        {
        }
        ...
        private void panel9_CloseClick(object sender, EventArgs e)
        {
        }
        ...
        bool takeoff_land = false;
        int count = -1;
        bool takeoff_land1 = false;
        private void but_takeoff_land_wp(object sender, EventArgs e)
        {
            ...
            if (a == null)
                a = "0";
            double  heading=double.Parse(a);

            sr.Close();

            takeoff_land1 = true;
          
            if (takeoff_land == false)
            {
                int count1 = Commands.Rows.GetLastRow(0);

                LonLatTest3 test = new LonLatTest3();
                double lon = double.Parse(TXT_homelng.Text);
                double lat = double.Parse(TXT_homelat.Text);
                double brng = (double)numericUpDown1.Value+heading;
                double dist = (double)numericUpDown4.Value;

                test.LonLat(lon, lat, brng, dist);
                ...
                InsertCommand(0, MAVLink.MAV_CMD.VTOL_TAKEOFF, 0, 0, 0, 0, double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (int)numericUpDown2.Value);//起飞


                InsertCommand(1, MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0, Longitude, Latitude, (int)numericUpDown3.Value);
                setfromMap(Latitude, Longitude, (int)numericUpDown3.Value);//引导
                ...
                if (state)
                {
                    test.LonLat(Longitude, Latitude, (double)numericUpDown1.Value+heading, 250);

                    
                    InsertCommand(2, MAVLink.MAV_CMD.LOITER_TO_ALT, 0, (double)numericUpDown6.Value, 0, 0, Longitude, Latitude, (int)numericUpDown7.Value);
                    setfromMap(Latitude, Longitude, (int)numericUpDown7.Value);//盘旋
                ...
                else
                {
                    ...
                    InsertCommand(2, MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0, Longitude, Latitude, (int)numericUpDown7.Value);
                    setfromMap(Latitude, Longitude, (int)numericUpDown7.Value);
                }
                la1 = Latitude;
                lo1 = Longitude;
                ...
                InsertCommand(3, MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0, Longitude, Latitude, (int)numericUpDown7.Value);
                setfromMap(Latitude, Longitude, (int)numericUpDown7.Value);//航高保持点
                la2 = Latitude;
                lo2 = Longitude;
                ...
                if (state1 == false)
                {
                    selectedrow = Commands.Rows.AddCopies(selectedrow, 1);
                    
                    Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                    ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                    setfromMap(la2 + 0.0001, lo2 + 0.0001, (int)numericUpDown5.Value);//返航点
                    if (state2)
                    {
                        InsertCommand(selectedrow + 1, MAVLink.MAV_CMD.LOITER_TO_ALT, 0, (double)numericUpDown6.Value, 0, 0, lo1 + 0.0001, la1 + 0.0001, (int)numericUpDown7.Value);
                        setfromMap(la1 + 0.0001, lo1 + 0.0001, (int)numericUpDown8.Value);//盘旋降高
                    }
                    else
                    {
                        ...
                        Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                        ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                        setfromMap(la1 + 0.0001, lo1 + 0.0001, (int)numericUpDown8.Value);
                    }
                    test.LonLat(double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (double)numericUpDown1.Value+heading, (int)numericUpDown10.Value);
                    selectedrow = Commands.Rows.AddCopies(selectedrow, 1);
                  
                    Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                    ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                    setfromMap(Latitude + 0.0001, Longitude + 0.0001, (int)numericUpDown11.Value);//降落引导高度
                ...
                else
                {
                    ...
                    test.LonLat(double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (double)numericUpDown1.Value+heading - 180, (int)numericUpDown10.Value + 500);
                    selectedrow = Commands.Rows.AddCopies(selectedrow, 1);
                   
                    Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                    ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                    setfromMap(Latitude, Longitude, (int)numericUpDown5.Value);//返航点

                    if (state2)
                    {
                        test.LonLat(double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (double)numericUpDown1.Value+heading - 180, (int)numericUpDown10.Value + 250);
                        InsertCommand(selectedrow + 1, MAVLink.MAV_CMD.LOITER_TO_ALT, 0, (double)numericUpDown6.Value, 0, 0, Longitude, Latitude, (int)numericUpDown8.Value);
                        setfromMap(Latitude, Longitude, (int)numericUpDown8.Value);//盘旋降高
                    }
                    else
                    {
                        test.LonLat(double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (double)numericUpDown1.Value+heading - 180, (int)numericUpDown10.Value + 250);
                        ...
                        Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                        ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                        setfromMap(Latitude, Longitude, (int)numericUpDown8.Value);
                    }

                    test.LonLat(double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), (double)numericUpDown1.Value+heading - 180, (int)numericUpDown10.Value);
                    selectedrow = Commands.Rows.AddCopies(selectedrow, 1);
                    
                    Commands.Rows[selectedrow].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                    ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
                    setfromMap(Latitude, Longitude, (int)numericUpDown11.Value);//降落引导高度
                }
                InsertCommand(selectedrow + 1, MAVLink.MAV_CMD.RETURN_TO_LAUNCH, 0, 0, 0, 0, double.Parse(TXT_homelng.Text), double.Parse(TXT_homelat.Text), 0);
                takeoff_land = true;
                count = Commands.Rows.GetLastRow(0) - count1;
            }
        ...
        private void but_3_dcheck(object sender, LinkLabelLinkClickedEventArgs e)
        {
        ...
        private void 航点_Click(object sender, EventArgs e)
        {
        ...
        {
            if (((double)numericUpDown3.Value - (double)numericUpDown2.Value) > 10 ||
                ((double)numericUpDown3.Value - (double)numericUpDown2.Value) < 0)
            {
        ...
        public class LonLatTest3
        {
             ...
             *            距离（米）
             */
            public void LonLat(double lon, double lat, double brng, double dist)
            {
        ...
        public void AddWPToMap0(double lat, double lng, int alt)
        {
            ...
            selectedrow = Commands.Rows.AddCopies(1, 1);
            //Commands.Focus();
            //SendKeys.Send("^{End}{Home}");
            //Commands.FirstDisplayedScrollingRowIndex = Commands.Rows[selectedrow].Index-1;
            ...
            Commands.Rows[1].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
            ChangeColumnHeader(MAVLink.MAV_CMD.WAYPOINT.ToString());
        ...
        private void myButton10_Click(object sender, EventArgs e)
        {
            ...
            {
                //drawnpolygonsoverlay.Markers.Clear();
                if ((takeoff_land == true) && (count > 0))
                {
                   
                    Commands.Rows.RemoveAt(3);
                    polygonsoverlay.Markers.Clear();
                    Commands.Rows.RemoveAt(2);
                    polygonsoverlay.Markers.Clear();
        ...
        bool wpinformantion = true ;
        bool wpinformantiontime = false;
        ...
        {
            wpinformantion = !wpinformantion;
        }

        private void checkBox5_CheckedChanged(object sender, EventArgs e)
        {
            wpinformantiontime = !wpinformantiontime;
        }


         
        //int linen = 0;
       /* private void Commands_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {

            {
                if (e.RowIndex >= Commands.FirstDisplayedScrollingRowIndex)
                {
                    using (SolidBrush b = new SolidBrush(Commands.RowHeadersDefaultCellStyle.ForeColor))
                    {
        ...
        //        int a = 0;
        //        int count = 0;
        //        //int b = Commands.Rows.GetLastRow(0);
        //        for (; a < linen1 + 1; a++)
        //        {
        //            double x = Math.Abs(Convert.ToDouble(Commands.Rows[a].Cells[Grad.Index].Value));
        //            //double x= Convert.ToDouble(Commands[Grad.Index, a].Value);
        //            if ((Commands.Rows[a].Cells[Command.Index].Value.ToString()) != MAVLink.MAV_CMD.LOITER_TO_ALT.ToString())
        //                if (x > 30)
        //                {
        //                    polygonsoverlay.Markers.Add(new GMarkerGoogle(new PointLatLng(Convert.ToDouble(Commands.Rows[a].Cells[Lat.Index].Value), Convert.ToDouble(Commands.Rows[a].Cells[Lon.Index].Value)), GMarkerGoogleType.red ));
        //                    //polygonsoverlay.Markers.Add(new GMarkerGoogle(new PointLatLng(Convert.ToDouble(Commands.Rows[a].Cells[Lat.Index].Value), Convert.ToDouble(Commands.Rows[a].Cells[Lon.Index].Value)), GMarkerGoogleType.blue));
        //                    count++;
        //                }
        //                else { }
        //            /*else {
                           ...
                           for (; a < b; a++)
                            {
                                ...
                                if ((Commands.Rows[a].Cells[Command.Index].Value.ToString()) == MAVLink.MAV_CMD.LOITER_TO_ALT.ToString())
                                {
                                    ...
                                    //addpolygonmarker("盘旋", Convert.ToDouble(Commands.Rows[a].Cells[Lon.Index].Value), Convert.ToDouble(Commands.Rows[a].Cells[Lat.Index].Value), Convert.ToDouble(Commands.Rows[a].Cells[Alt .Index].Value),Color.Blue);
                                    rallypointoverlay.Markers.Add(new GMarkerGoogle(new PointLatLng(Convert.ToDouble(Commands.Rows[a].Cells[Lat.Index].Value), 
                                        Convert.ToDouble(Commands.Rows[a].Cells[Lon.Index].Value)), GMarkerGoogleType.blue));
                                    //rallypointoverlay.Markers.Clear();
                                }
                                else if (x > 30)
                                {
                                    polygonsoverlay.Markers.Add(new GMarkerGoogle(new PointLatLng(Convert.ToDouble(Commands.Rows[a].Cells[Lat.Index].Value),
                                        Convert.ToDouble(Commands.Rows[a].Cells[Lon.Index].Value)), GMarkerGoogleType.red_dot));
                                    count++;
                                }
        ...
        //int count1 = 0;
        private PointLatLng Fristpoint;
        private PointLatLng Secondpoint;

        
        /*private void MainMap_MouseDoubleClick(object sender, EventArgs e)
        {

            if (a == 0)
            {
                a++;
                Fristpoint = MouseDownStart;
                polygonsoverlay.Markers.Add(new GMarkerGoogle(MouseDownStart, GMarkerGoogleType.blue));
            }
            else
            {
                a++;
                Secondpoint = MouseDownStart;
                polygonsoverlay.Markers.Add(new GMarkerGoogle(MouseDownStart, GMarkerGoogleType.blue));
            }
            ...
            if (a == 2)
            {
                CustomMessageBox.Show("距离: " +
                                        FormatDistance(
                                            MainMap.MapProvider.Projection.GetDistance(Fristpoint, Secondpoint), true) +
                                        " 方位角: " +
                                        ((MainMap.MapProvider.Projection.GetBearing(Fristpoint, Secondpoint))
                                            .ToString("0")));
        ...
        private void myButton7_Click_1(object sender, EventArgs e)
        {
            Form1 frm = new Form1();
            frm.Show();
        }
        ...
        private static bool SaveBinaryFile(WebResponse response, string FileName)
        {
            ...
            try
            {
                ...
                Stream outStream = System.IO.File.Create(Settings.GetUserDataDirectory()+"network.kml");
        ...
        private void myButton8_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string sw = "";
                    //byte[] buffer = new byte[1090];
                    bool xx = false;
                    fd.Filter = "All Supported Types|*.kml;";
                    DialogResult result = fd.ShowDialog();
                    string file = fd.FileName;
                    string outfilename = Path.GetDirectoryName(file) + Path.DirectorySeparatorChar +
                                         Path.GetFileNameWithoutExtension(file) + ".poly";
                    // int x=file.Length;
                    ...
                    while (!ReaderFile.EndOfStream)
                    {
                        int index1 = -1;
                        int index2 = -1;
                        string strRead = ReaderFile.ReadLine();
                        // strRead = strRead.Replace(" ", "");
                        int index = strRead.IndexOf("\t\t\t1");
                        if (index == -1) index = strRead.IndexOf("\t\t\t3");
                        if (index == -1) index = strRead.IndexOf("\t\t\t0");
                        if (index == -1) index = strRead.IndexOf("\t\t\t2");
                        if (index == -1) index = strRead.IndexOf("\t\t\t3");
                        if (index == -1) index = strRead.IndexOf("\t\t\t4");
                        if (index == -1) index = strRead.IndexOf("\t\t\t5");
                        if (index == -1) index = strRead.IndexOf("\t\t\t6");
                        if (index == -1) index = strRead.IndexOf("\t\t\t7");
                        if (index == -1) index = strRead.IndexOf("\t\t\t8");
                        if (index == -1) index = strRead.IndexOf("\t\t\t9");

                        index1 = strRead.IndexOf("<coordinates>1");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>0");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>2");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>3");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>4");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>5");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>6");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>7");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>8");
                        if (index1 == -1) index1 = strRead.IndexOf("<coordinates>9");

                        index2 = strRead.IndexOf("  1");
                        if (index2 == -1) index2 = strRead.IndexOf("  0");
                        if (index2 == -1) index2 = strRead.IndexOf("  2");
                        if (index2 == -1) index2 = strRead.IndexOf("  3");
                        if (index2 == -1) index2 = strRead.IndexOf("  4");
                        if (index2 == -1) index2 = strRead.IndexOf("  5");
                        if (index2 == -1) index2 = strRead.IndexOf("  6");
                        if (index2 == -1) index2 = strRead.IndexOf("  7");
                        if (index2 == -1) index2 = strRead.IndexOf("  8");
                        if (index2 == -1) index2 = strRead.IndexOf("  9");
                        ...
                        if (index > -1 || index1 > -1 || index2 > -1)
                        {
                            ...
                            if (index2 == -1)
                            {
                                ...
                                strwrite = strRead.Replace("\t\t\t\t\t\t", "#saved by Mission Planner 1.3.62\n");
                                strwrite = strwrite.Replace("<coordinates>", "");
                                strwrite = strwrite.Replace("0 ", "\n");
                                strwrite = strwrite.Replace(",", " ");
                                strwrite = strwrite.Replace("</coordinates>", "");
                                string WriteFilePath = outfilename;
                                StreamWriter WriteFile = File.CreateText(WriteFilePath);
                                WriteFile.WriteLine(strwrite);
                            ...
                            else
                            {
                                ...
                                sw += data[0] + " " + data[1] + "\n";
                                string wp = outfilename;
                                StreamWriter WriteFile1 = File.CreateText(wp);


                                WriteFile1.WriteLine(sw); WriteFile1.Close();
                                xx = true;
                            }
                        }
                        //if (xx == 1&& index==0) break;
                    }
                    ReaderFile.Close();
                    StreamReader sr;
                    if (xx == false)
                        sr = new StreamReader(outfilename);
                    else sr = new StreamReader(outfilename);
                    ...
                    while (!sr.EndOfStream)
                    {
                        ...
                        else
                        {
                            ...
                            addpolygonmarkergrid(drawnpolygon.Points.Count.ToString(),
                                double.Parse(items[0], CultureInfo.InvariantCulture),
                                double.Parse(items[1], CultureInfo.InvariantCulture), 0);
            ...
            try
            {
                //FileInfo finfo = new FileInfo(outfilename);
                //finfo.Delete();
                 //finfo = new FileInfo(@"C:\Users\Administrator\Desktop\1.poly");
                //finfo.Delete();
            }
            catch {
        ...
        private void 加载多边形kmlToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }

        private void 基础设置_Resize(object sender, EventArgs e)
        {
            基础设置.Refresh();
        }
        ...
        private void myButton9_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog fd = new OpenFileDialog())
            {
                ...
                while (!ReaderFile.EndOfStream)
                {
                    ...
                    if (index > -1 || index1 > -1 || index2 > -1)
                    {
                        ...
                        if (index2 == -1)
                        {
                            ...
                            strwrite = strwrite.Replace(",", " ");
                            strwrite = strwrite.Replace("</coordinates>", "");
                            string WriteFilePath = @"C:\Users\Administrator\Desktop\" + fn.ToString() + ".poly";
                            StreamWriter WriteFile = File.CreateText(WriteFilePath);
                            WriteFile.WriteLine(strwrite);
                            fn++;
                            WriteFile.Close();
                        ...
                        else
                        {
                            ...
                            string[] data = strRead.Split(',');
                            sw += data[0] + " " + data[1] + "\n";
                            string wp = @"C:\Users\Administrator\Desktop\" + fn.ToString() + ".poly";
                            StreamWriter WriteFile1 = File.CreateText(wp);
                ...
                for (; fn > nn; nn++)
                {
                    ...
                    if (xx == false)
                    {
                        sr = new StreamReader(@"C:\Users\Administrator\Desktop\" + nn.ToString() + ".poly"); 
                    }
                    else { sr = new StreamReader(@"C:\Users\Administrator\Desktop\" + nn.ToString() + ".poly");  }
                    ...
                    while (!sr.EndOfStream)
                    {
                        ...
                        else
                        {
                            ...
                            { drawnpolygon.Points.Add(new PointLatLng(
                                double.Parse(items[1], CultureInfo.InvariantCulture),
                                double.Parse(items[0], CultureInfo.InvariantCulture))); wj = true;
                        }
                            addpolygonmarkergrid(drawnpolygon.Points.Count.ToString(),
                                double.Parse(items[0], CultureInfo.InvariantCulture),
                                double.Parse(items[1], CultureInfo.InvariantCulture), 0);
                    ...
                    FileInfo finfo = new FileInfo(@"C:\Users\Administrator\Desktop\" + nn.ToString() + ".poly");
        ...
        private void 修改高度ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string altdif = "0";
        ...
        private void myButton14_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("如果你在现场，连接到你的APM，等待GPS锁定。然后单击“起始位置”将Home点设置为您的位置","提示");
            }
        }
                ...
                try
                {
                    ...
                    while (!error && !sr.EndOfStream)
                    {
                        ...
                        try
                        {
                            ...
                            if (Convert.ToInt32(items[0]) <= 4&& Convert.ToInt32(items[0])!=0)
                                InsertCommand(Convert.ToInt32(items[0])-1, (MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), items[3], false),
                                    0, double.Parse(items[5]), double.Parse(items[6]), double.Parse(items[7]), 
                                    double.Parse(items[9]), double.Parse(items[8]), double.Parse(items[10]));
                            else if(Convert.ToInt32(items[0]) > Length-7)

                                InsertCommand(Commands.RowCount, (MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), items[3], false),
                                    0, double.Parse(items[5]), double.Parse(items[6]), double.Parse(items[7]),
                                    double.Parse(items[9]), double.Parse(items[8]), double.Parse(items[10]));
        ...
        {
            int i = 1;
            double/* Longitude = 0, Latitude = 0,*/alt=0;
            int cloumns=Commands.RowCount;
            LonLatTest3 test = new LonLatTest3();
            for (; i < cloumns-1; i++)
            {
                if (double.Parse(Commands.Rows[i].Cells[AZ.Index].Value.ToString ()) > 20) 
                { 
                    Longitude = (double.Parse(Commands.Rows[i].Cells[Lon.Index].Value.ToString()));
                    Latitude = double.Parse(Commands.Rows[i].Cells[Lat.Index].Value.ToString());
                    alt = double.Parse(Commands.Rows[i].Cells[Alt.Index].Value.ToString());
                    test.LonLat(Longitude, Latitude, double.Parse(Commands.Rows[i+1].Cells[AZ.Index].Value.ToString()) + 180, 150);

                    /*Commands.Rows.Insert(i);
                    Commands.Rows[i].Cells[Command.Index].Value = MAVLink.MAV_CMD.WAYPOINT.ToString();
                    Commands.Rows[i].Cells[TagData.Index].Tag = null ;
                    Commands.Rows[i].Cells[TagData.Index].Value = null;
                    Commands.Rows[i].Cells[Param1.Index].Value = 0;
                    Commands.Rows[i].Cells[Param2.Index].Value = 0;
                    Commands.Rows[i].Cells[Param3.Index].Value = 0;
                    Commands.Rows[i].Cells[Param4.Index].Value = 0;
                    Commands.Rows[i].Cells[Lat.Index].Value = Longitude;
                    Commands.Rows[i].Cells[Lon.Index].Value = Latitude;
                    Commands.Rows[i].Cells[Alt.Index].Value = alt;*/

                    InsertCommand(i, MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0, Longitude, Latitude, alt);
                    i = i+1;
                    cloumns = cloumns + 1;


                    //setfromMap(Latitude, Longitude, (int)alt);
                }
            }
        }
    }
